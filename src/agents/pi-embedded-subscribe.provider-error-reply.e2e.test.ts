...
// OB-54 (Codex #159 r1/r2): a provider refusal inside a 200-shaped reply must
// be judged on the COMPLETE message and never reach the channel — not as a
// whole, not as streamed fragments, not via the message_end path.
const CODEX_REFUSAL =
...
function endMessage(emit: (evt: unknown) => void, text: string) {
...
describe("subscribeEmbeddedPiSession — provider error reply (OB-54)", () => {
  it("a refusal streamed in fragments longer than the chunk size never reaches onBlockReply", () => {
    ...
    for (let i = 0; i < CODEX_REFUSAL.length; i += 17) {
      emitAssistantTextDelta({ emit, delta: CODEX_REFUSAL.slice(i, i + 17) });
    }
    emitAssistantTextEnd({ emit, content: CODEX_REFUSAL });
    endMessage(emit, CODEX_REFUSAL);

    expect(onBlockReply).not.toHaveBeenCalled();
    expect(onPartialReply).not.toHaveBeenCalled();
    expect(subscription.getRawErrorReply()).toMatch(/not supported when using Codex/);
  ...
  it("the message_end delivery path (no deltas) is gated too", () => {
    ...
    expect(onBlockReply).not.toHaveBeenCalled();
    expect(subscription.getRawErrorReply()).toMatch(/not supported/);
  });
  ...
  it("prose still streams chunk by chunk", () => {
    const { emit, onBlockReply } = harness();
    emitAssistantTextDelta({ emit, delta: "First line\nSecond line\nThird line\n" });
