...
| openclaw           | chore/ci-format-and-install-smoke-20260507 | TBD | in_progress     | Claude  | oxfmt fixes (5 files), install-smoke run.sh | format:check + commit hooks | push branch, open PR                     | Unblocks CI for all open PRs by clearing pre-existing oxfmt violations on main and updating install-smoke to parse the new `OpenClaw X.Y.Z (<hash>)` --version output |
