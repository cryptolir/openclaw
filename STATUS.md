...
- Claude owns `openclaw-dashboard` branch `claude/docs-axonfi-vault-plan` (PR #64) for AgentGlob × AxonFi vault integration plan; awaiting review before Phase 1 code.
...
| openclaw-dashboard | claude/docs-axonfi-vault-plan       | #64 | review          | Claude  | AXONFI_VAULT_INTEGRATION.md (planning doc only) | docs only          | Codex/owner review of locked decisions, spot-checks, and PR plan | Plan for AgentGlob × AxonFi vault integration; bot keys via Cloud KMS, owner ops via MetaMask, manage_vaults shared by owner+admin, runtime routes CORS-isolated, Base Sepolia first |
