...
- **What changed**: `GET|POST /api/agents/[agentId]/config` and `POST /api/community/groups/[id]/apply`
  ...
  `openclaw.json` whenever the per-agent file was missing, on nothing but `test -f`, behind a
  `write_agents` check non-owner Org admins pass. Both now resolve ONLY the agent's own file:
  legacy → 400 before any SSH (pointer to the System tab), missing file → the control route's
  existing 409 ("Run Deploy"), and no arm names the host-global path. Two tests in
  `lib/agent-server.test.ts`: a source sweep over every route under `app/api/agents` +
  `app/api/community`, and an order/status pin. Both fail against main's version of either route.
...
- **Housekeeping**: dropped the #429 and #131 rows from Active Branches — both merged 2026-08-22.
  ...
  Left untouched; owner to remove or reuse. 46 dashboard PRs merged since the previous entry with
  no STATUS.md record — the open-PR list plus the `writer-lock` label is the working ownership map.
