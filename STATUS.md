...
- Claude owns `openclaw-dashboard` branch `claude/docs-hot-wallet-plan` (PR #65) for per-agent EVM hot wallet plan (v1).
- Earlier AxonFi vault plan (`claude/docs-axonfi-vault-plan`, PR #64) deferred as v2 backlog; tracked in ROADMAP.md.
...
| openclaw-dashboard | claude/docs-hot-wallet-plan         | #65 | review          | Claude  | HOT_WALLET_INTEGRATION.md (planning doc only)   | docs only          | Codex/owner review of v1 hot-wallet plan  | Per-agent EVM hot wallet (Eth/Arb/Polygon/Base); paste private key, AES-GCM encrypted at rest, default Alchemy RPC, no policy gates |
| openclaw-dashboard | claude/docs-axonfi-vault-plan       | #64 | deferred (v2)   | Claude  | AXONFI_VAULT_INTEGRATION.md (planning doc only) | docs only          | Reactivate when v1 risk envelope outgrown | Non-custodial AxonFi vault + Cloud KMS bot keys; deferred to v2, see ROADMAP.md Wallet & Payments                                   |
