...
| ------------------ | ----------------------------------- | ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| openclaw           | fix/disk-alarm-sized-to-image-pull  | #165 | merged 630191bc7e8 (2026-09-23) — **the disk alarm fired too late and the prune's result never reached anyone.** P0 was `>=90% used`, i.e. 7.5 G free on a 75 G disk — already under one ~8.7 G image pull; the prune ran AFTER the scan and printed only to /var/log, so "removed 0 tag(s)" against a 92% disk went unread for 10 days. | Claude | scripts/ops/{agents_server_diagnostic,diagnostic-cron,prune-gateway-images}.sh, STATUS.md | bash -n x3; scan --no-write vs both hosts (no false alarm at 36/33 G free; forced thresholds → US P0 + EU P2); prune exit 0 at the real floor, exit 3 + plain message at a raised one; cron block + subject + body run verbatim against both hosts in forced dry-run | owner merge → 05:40 re-pin, 06:00 run uses it (no host step) | Thresholds are now GB FREE (P0 <10, P2 <20), not % used. The prune moved BEFORE the scan, so a disk finding means "still short after cleanup". Prune output is in the email; exit 3 (still <10 G free) puts `⚠ DISK — next image pull will fail` in the SUBJECT. `2>/dev/null` on the prune dropped. MIN_FREE_GB duplicates DISK_FREE_CRIT_GB — comments in both say change both. |
| openclaw           | fix/prune-all-openclaw-image-repos  | #164 | merged d871f2cc56a (2026-09-23) — **disk prune was scoped to ONE hard-coded repo path** (`REPO=...openclaw-gateway/gateway`), so every other openclaw image repo was invisible to it and nothing ever pruned the build cache. EU hit 92% full on 2026-09-23 with ~25 G the daily prune could not see, while reporting success every day. | Claude | scripts/ops/prune-gateway-images.sh | bash -n; dry-run on BOTH live hosts (kept everything, removed nothing); throwaway-repo test on EU proved discovery + per-repo recency + real removal, trap-cleaned | owner merge; the 06:00 cron picks it up next day via the 05:40 re-pin (no host step). `/opt/openclaw` on each host is a separate copy used by deploy.sh pre-pull — see OB-22 before pulling there | Repos are now discovered at run time (any repository whose name contains `openclaw`) instead of named, so a new image repo is covered the first time it appears. Keep policy unchanged: in-use + KEEP_RECENT most-recent PER REPO. Adds `docker builder prune -af` (EU carried 6.1 G, 25 entries, none active). Journals are NOT in scope — they need `SystemMaxUse` in journald.conf, a host config change. |
| openclaw           | feat/exec-outreach-session-identity | #160 | open — **Ceyo Phase 2, C2/C10/C11/C17**: at the exec merge, strip `OPENCLAW_SESSION_KEY`/`OUTREACH_CRON_JOB`/`OUTREACH_ROLE` from the model's `params.env` and re-assert the runtime's values after it; Compose passes `OUTREACH_CRON_JOB` + five provider keys to both services. **Must merge and roll BEFORE dashboard Phase 2 merges** or the two Phase 1 agents stop writing. | Claude | src/agents/bash-tools.exec.ts, bash-tools.exec-runtime.ts (+test), docker-compose.yml, STATUS.md                                                                                                                            | vitest 6/6 new + 5/5 neighbours; tsc clean in src/                 | owner merge → `build-and-push.sh` (let it pick the tag) → pull /opt/openclaw on DevAgents → re-provision scout + qualifier → then dashboard Phase 2 PR (runbook §0)                                                                                 | Six vitest cases incl. a wiring assertion on the source. T15 already caught the read jail (dashboard #543).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| openclaw           | fix/ob54-bare-error-reply-failover  | #159 | open — **OB-54 fix**: a provider that refuses INSIDE a 200-shaped reply (the assistant "answer" is a bare error object such as `{"detail":"…"}`) now throws `FailoverError` so the configured `fallbacks` engage; with no fallbacks it is delivered as an error payload, never as the answer.                                                                                     | Claude | src/agents/pi-embedded-helpers/errors.ts (bare `detail`/`error` bodies count as error payloads; `describeRawErrorReply`), pi-embedded-helpers.ts (re-export), pi-embedded-runner/run.ts (the hook), new e2e test, STATUS.md | vitest: new test + existing helper/payload tests; tsc              | owner merge → `build-and-push.sh` (let it pick the tag) → pilot on projectmanager                                                                                                                                                                   | Measured 2026-09-07 and 2026-09-08 on projectmanager: OpenAI retired `gpt-5.4-mini` for ChatGPT accounts mid-afternoon and every Telegram turn showed the raw `{"detail":…}` JSON; Venice was never called. Keyed on the payload shape, not Codex's wording.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| openclaw           | feat/codex-forward-compat-any-id    | #156 | merged fb7f44ed4 → image **v2026.9.7.1** (sha fb7f44ed4a1) built; **piloted on projectmanager (2ndClaw) 2026-09-07: `openai-codex/gpt-5.4-mini` answers a real turn on the ChatGPT subscription, Venice fallbacks kept.** Fleet roll NOT done (owner decision). Release registration returned 409 — register/promote by hand.                                                     | Claude | src/agents/model-forward-compat.ts (+test), STATUS.md                                                                                                                                                                       | vitest 6/6 on the file                                             | owner merge → build-and-push.sh → pilot on projectmanager (2ndClaw) only, then set openai-codex/gpt-5.4-mini with Venice fallbacks                                                                                                                  | OpenAI retired every Codex id the bundled catalog knows (gpt-5.1..5.3) for ChatGPT-account users — measured 2026-09-07 on projectmanager, whose `openai-codex:default` OAuth profile is valid: all seven ids answer `not supported when using Codex with a ChatGPT account`, current ids (gpt-5.4-mini, gpt-5.5, gpt-6-astra, from the owner's Codex CLI model cache) are `Unknown model`, and `models.providers` cannot add them because the config schema has no `openai-codex-responses` api value (tried; the invalid config restart-looped the agent). Fix: the 5.3-only forward-compat fallback now covers ANY unknown openai-codex id (clone newest bundled codex template, else synthesize against chatgpt.com/backend-api). Also filed: the Venice fallback does not engage on these refusals because they arrive as a 200-shaped reply body. |
| openclaw           | feat/codex-home-persist             | #155 | merged a7ada83c0; pulled on 2ndClaw, projectmanager recreated                                                                                                                                                                                                                                                                                                                     | Claude | docker-compose.yml (both env blocks: CODEX_HOME), STATUS.md                                                                                                                                                                 | docker compose config parses; CODEX_HOME rendered in both services | owner merge, then `git pull` /opt/openclaw on 2ndClaw and force-recreate projectmanager; owner runs the Codex login inside the container                                                                                                            | openclaw reads the ChatGPT-subscription login from `$CODEX_HOME/auth.json`, default `~/.codex` — not a mount, so a login inside the container died on the next recreate. Now `/home/node/.openclaw/.codex`, inside the config-dir bind. Hardcoded (not a secret, same for every agent) so it is not a docker.env key and the OB-22 drift check has nothing to compare. Found while moving projectmanager to `openai-codex/*`: no login present, both Codex models refused, and the Venice fallback did NOT engage because the refusal arrives as a 200-shaped reply. **Measured after merge: the fleet build (2026.2.19 / v2026.9.4.1) does not read CODEX_HOME at all — inert until a build that does. The Codex OAuth login lives in `agents/main/agent/auth-profiles.json`, inside the mount, so it already persisted.**                            |
...
| openclaw           | fix/dependabot-registry-schema      | #153 | in review                                                                                                                                                                                                                                                                                                                                                                         | Claude | .github/dependabot.yml, .github/workflows/docker-release.yml, STATUS.md                                                                                                                                                     | yaml parses; actionlint; main CI already red (#126)                | owner merge (squash); then mark this row merged in the next STATUS.md PR                                                                                                                                                                            | Twin of openclaw-dashboard#513 (OB-55): the credential-less `npm-registry` block is invalid — Dependabot requires a token or username+password for that type, and this repo has 0 Dependabot secrets, so no token is possible; `registry.npmjs.org` is the npm default. Fork ⇒ GitHub gives no config verdict here; evidence is the parse plus a diff byte-identical to the proven dashboard fix (GitHub check `success` on its main, Dependabot opened dashboard#514 41 s later). Also adds `.github/dependabot.yml` to `docker-release.yml` paths-ignore so a config-only edit stops pushing a ghcr image. #146–#148 here are _security_ updates (`/ui` npm, go_modules — neither is in this config), not version updates.                                                                                                                           |
| openclaw           | chore/node-24-lts                   | #154 | in review                                                                                                                                                                                                                                                                                                                                                                         | Claude | Dockerfile, .github/workflows/{ci,install-smoke}.yml, .github/actions/setup-node-env/action.yml, .github/dependabot.yml, STATUS.md                                                                                          | gateway image built on node:24 on DevAgents; CI on 24              | owner: merge, then decide the fleet release (gateway tag)                                                                                                                                                                                           | Gateway runtime to **Node 24 LTS**, mirroring dashboard#525: `Dockerfile` base `node:24-bookworm` (digest-pinned, resolves to v24.20.0), CI + `setup-node-env` default + install-smoke to `24.x`, Dependabot ignores major `node` image bumps. `engines` stays `>=22.12.0` (upstream range, already permits 24). **Repo only — the fleet still runs v22.22.0 on `gateway:v2026.9.1.1`/`v2026.9.4.1` until an owner-approved release**: `build-and-push.sh <tag>` then `deploy.sh <tag>` (one agent at a time, health-checked, auto-rollback). CI test jobs are pre-existing red (#126).                                                                                                                                                                                                                                                                |

---

## Last Session

- **Date**: 2026-09-23 (fleet disk — EU 92%, US 89%, DevAgents 92% while the daily image prune reported success; owner: Claude)
- **Repo**: openclaw — **#164** (`d871f2cc56a`): the prune matched ONE hard-coded repo, so ~25 G of legacy local
  `openclaw:*` builds, the hermes repo and the build cache were invisible to it; it now sweeps every `openclaw` image
  repo and runs `docker builder prune`. **#165** (`630191bc7e8`): the disk alarm is in GB free (P0 under 10 G = the next
  ~8.7 G pull fails; P2 under 20 G), not `>=90% used` (= 7.5 G free, already too late); the prune runs BEFORE the scan,
  its output goes into the email, and exit 3 puts `⚠ DISK — next image pull will fail` in the subject. Dashboard
  **#632**: bug_list Resolved row + CLAUDE.md §8 (gateway deploys run from DevAgents).
- **Hosts, by hand — recorded here because no repo holds them**: unused images + build cache removed on all three
  (EU 92→49%, US 89→54%, DevAgents 92→42%; every removed registry tag was first confirmed present in Artifact
  Registry; the three legacy local `openclaw:*` builds on EU+US were deleted on owner OK). journald `SystemMaxUse=2G`
  on all three (backups `/etc/systemd/journald.conf.bak.pre-maxuse-*`). DevAgents
  `/opt/openclaw-ops/scripts/prune-gateway-images.sh` is now a **symlink** to `.openclaw-autoscan` — the hand copy
  `deploy.sh` streamed had been stale since 2026-06-20 (backup `.bak.pre-autoscan-link-20260923`). Do not replace it
  with a copy. `.openclaw-autoscan` was re-pinned to `630191bc7e8` early, with the same command as its 05:40 line.
- **Validation**: every changed path was run against the live hosts without writing — dry-runs, forced thresholds
  (US→P0, EU→P2 in one run), prune exit 3 at a raised floor, and the cron's new block + subject/body lifted verbatim
  from the file. `main` is byte-identical to the tested commits. CI: the 4-check red baseline only.
- **Next**: 06:00 UTC 2026-09-24 is the first natural run of #164 + #165 — its email should carry an IMAGE PRUNE
  section and no `⚠ DISK`. Owner's call: `hermes-agent:v1-verify` on DevAgents (3.85 G, unused, the only copy).
  The 1stClaw/2ndClaw `/opt/openclaw-ops` dirs are March leftovers that never prune — left in place, now documented.

- **Date**: 2026-09-06 (dashboard: #200 landed after a two-month stall; Dependabot config fixed in both repos; owner: Claude)
- **Repo**: openclaw-dashboard.
  - **#200** `feat(reports)` (`37fe504`, rev `00535-8kk`, tag `v2026.9.6.1`): platform-admin **Top Token Consumers** — every agent
    across all Orgs ranked by tokens, 24h/7d/30d/90d, top model per agent. The July draft was merge-clean and still did not compile:
    #470 had added `strictWindow` to `loadAgentReport` (passed `false` — live UI path, no ledger writes); the Unreachable card now reads
    `unreachableAgentCount`, not `failedAgentCount` (which also counts hermes agents that never report usage); coverage reuses
    `WorkspaceCoverage`. Owner still to eyeball the live table (no session can log in).
  - **#513** `fix(security)` (`623ca95`): `.github/dependabot.yml` was invalid since #499 — a credential-less `npm-registry` block —
    so **version updates never ran**; the existing Dependabot PRs (#500–#508) are _security_ updates, a separate feature. GitHub's
    check on main → `success`; Dependabot opened **#514** 41 s later (docker group, **node 22 → 26 MAJOR** — owner decision, not merged).
    Filed as OB-55. Measured: the config check runs only on commits touching the file, surfaces only failures on PR branches (6 s),
    gives a real verdict only on main (~1 min), never on forks.
- **Repo**: openclaw — branch `fix/dependabot-registry-schema` (row above): the identical block removed (8 deletions) plus
  `.github/dependabot.yml` added to `docker-release.yml` paths-ignore. Fork ⇒ no GitHub verdict; evidence is the parse plus the
  byte-identical diff to #513.
- **Repo**: openclaw — branch `chore/node-24-lts` (row above): gateway `Dockerfile` + CI to Node 24 LTS, Dependabot major-ignore. Repo
  only; fleet rollout is a separate owner decision (`build-and-push.sh` → `deploy.sh`).
- **Also surfaced**: #470's provider-billed Venice column is inert — `VENICE_ADMIN_API_KEY` is unbound on Cloud Run (env-name grep
  = 0), so every daily usage email is `[PARTIAL]` (owner-only Secret Manager step, runbook `docs/ops/provider-usage-report.md`);
  the shared dashboard checkout's `node_modules` is stale vs main (missing `@types/js-yaml`) and fakes a broken build when symlinked
  into a worktree — always `npm ci` in the worktree; EU fleet on gateway `v2026.9.1.1` while US is on `v2026.9.4.1`.
- **Next concrete steps**: owner: eyeball Platform Reports; decide dashboard#514; bind `VENICE_ADMIN_API_KEY`; #444/#425/#426/#432
  unchanged; #449 still `writer-lock` (another session's — hands off).
