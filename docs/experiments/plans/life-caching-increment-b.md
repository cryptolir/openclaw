<!-- Status: design proposal — review-only (Codex). rev4 addresses reviews 4536945369 + 4537042419 + 4537287788. Follow-up to #77 (rev4) after Increment A (#80) shipped; memo design reconciled with shipped fix #83. Not yet implemented; do NOT merge to deploy. -->
...
- **[P2] Memoization scope must exclude bootstrap hooks.** Confirmed: `resolveBootstrapFilesForRun` runs `applyBootstrapHookOverrides(...)` before returning, so memoizing the whole `resolveBootstrapContextForRun` on session/config/app-file keys would **freeze hook-generated context** for the session. Fix: memoize only the static load/filter span and re-run hooks every turn. _(rev4 below narrows the memoized span to `loadWorkspaceBootstrapFiles` → `filterBootstrapFilesForSession` and moves `applyAppBootstrapVariants` to after hooks, matching shipped #83; memo key covers all canonical inputs.)_ Verification adds a **dynamic-hook freshness test** (hook output changes turn-to-turn must still surface), not just `AGENTS.app.md` edits.
...
## Review response — Codex review 4537287788 (rev4)

Two P2s, both folded in. Note: the #82 review landed in parallel and **moved app bootstrap shaping to AFTER hooks** (shipped as fix #83) — so the memo design is updated to match.

- **[P2] cached-tokens lives in a nested usage detail.** Confirmed: OpenAI-compatible chat completions report `usage.prompt_tokens_details.cached_tokens`, and this repo's Responses mock uses `usage.input_tokens_details.cached_tokens` (`test-helpers.openai-mock.ts`). Reading top-level `usage.cached_tokens` would always be `undefined` → a false "no cache" → wrong Phase 3 decision. Phase 0 now requires the **nested** field (or the SDK-normalized cache-read value), checking both the chat-completions and Responses shapes.
- **[P2] Memo key must cover all bootstrap inputs.** Confirmed: the memoized span starts with `loadWorkspaceBootstrapFiles`, which reads **canonical** `BOOTSTRAP.md` / `AGENTS.md` / `SOUL.md` (the last two when a variant is absent). The earlier key (app-variant mtimes + config) would serve those **stale** mid-session. Fix (also reconciled with #83's post-hook ordering): memoize **only `loadWorkspaceBootstrapFiles` → `filterBootstrapFilesForSession`**, keyed on **all selected canonical file mtimes+sizes/hashes + variant present/missing state + sessionKey + promptMode + config version**. `applyAppBootstrapVariants` now runs **after hooks** (per #83) and is **not memoized**, so editing an `AGENTS.app.md` variant is reflected automatically (re-read each turn) without needing it in the memo key. Tests add an **edit-canonical-file** (e.g. `BOOTSTRAP.md`) invalidation case alongside the variant-edit and dynamic-hook cases.
...
1. **Does the provider cache a stable prefix at all?** (the gate — decides whether Phase 3 is automatic or needs Option A/B.) Capture the model response's **nested** cached-token count on turn 2+ of a single app session whose prefix is unchanged — `usage.prompt_tokens_details.cached_tokens` (chat completions) **or** `usage.input_tokens_details.cached_tokens` (Responses), or the SDK-normalized cache-read value (codex 4537287788 — the top-level `usage.cached_tokens` does not exist and would read a false zero). > 0 ⇒ caches implicitly; 0 ⇒ it does not. Add a one-line log of that nested field + prompt tokens at the openai-completions response boundary (token counts only — no content).
...
2. **Per-session memoization of the STATIC canonical-load span only** (Codex 4536644504 #3 + 4536945369 P2 + 4537287788): memoize **just** `loadWorkspaceBootstrapFiles` → `filterBootstrapFilesForSession` (the canonical file I/O). **Everything dynamic runs every turn after it, in this order (matching shipped #83): `applyBootstrapHookOverrides` → `applyAppBootstrapVariants` → APP_PROFILE** — so hook output, the `.app.md` swap, and per-user context are never frozen. The memo key must cover **every bootstrap input that feeds the span**, not just variants: `(sessionKey, promptMode, config version, AND each selected canonical bootstrap file's mtime+size [or content hash] — BOOTSTRAP.md / AGENTS.md / SOUL.md / etc.)`. Because `applyAppBootstrapVariants` is **post-hook and un-memoized**, editing an `AGENTS.app.md`/`SOUL.app.md` variant is reflected automatically (re-read each turn) — the variant files need not be in the memo key. Conservative fallback = new-session-only / short TTL. Tests: (a) edit a **canonical** file (`BOOTSTRAP.md`) mid-session → next turn reflects it (memo key covers canonical inputs); (b) edit `AGENTS.app.md` mid-session → next turn reflects it (variant re-read, not memoized); (c) **dynamic-hook freshness** — a bootstrap hook whose output changes turn-to-turn still surfaces every turn.
...
- `src/agents/bootstrap-files.ts` — remove APP_PROFILE from the bootstrap-file list; memoize **only `loadWorkspaceBootstrapFiles` → `filterBootstrapFilesForSession`** keyed on all selected **canonical** file mtimes/hashes + sessionKey + promptMode + config version; `applyBootstrapHookOverrides` → `applyAppBootstrapVariants` → APP_PROFILE all run **after** the memo, every turn (post-hook ordering per shipped #83) (P2).
...
- **Cross-turn stability (the Increment B win)**: `sharedPrefixDigest` is constant turn-to-turn within a session (memoization). Memo invalidation covers **all inputs**: editing a **canonical** file (`BOOTSTRAP.md`) → next turn reflects it (canonical mtimes/hashes in the key); editing an `AGENTS.app.md` variant → next turn reflects it (variant re-read post-hook, not memoized).
